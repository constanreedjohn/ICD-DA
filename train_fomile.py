...
class trainer(object):
    ...
    def dataloader(self, opt):
        ...
        if opt.sub_valid_data is not None:
            sub_valid_dataset = hierarchical_dataset(root=opt.sub_valid_data, opt=opt)
            sub_valid_loader = torch.utils.data.DataLoader(
                sub_valid_dataset, batch_size=opt.batch_size,
                shuffle=True,
                num_workers=(int(opt.workers)),
                collate_fn=AlignCollate_valid, pin_memory=True
            )
            return src_train_dataset, tar_train_dataset, valid_loader, sub_valid_loader
        else:
    ...
    def build_model(self, opt):
        ...
        if 'CTC' in opt.Prediction:
            ...
            if opt.loss_type == 'CELoss':
                self.criterion = torch.nn.CrossEntropyLoss(ignore_index=0).to(device)
            elif opt.loss_type == 'FocalLoss':
                self.criterion = FocalLoss(gamma=opt.loss_focal_gamma).to(device)
            else: raise ValueError(f"{opt.loss_type} not Implemented")
    ...
    def entropy_focal(self, p_softmax, alpha=0.2, gamma=1):
        # Ref: pytorch BCE
        ce = - torch.mul(p_softmax, torch.log(p_softmax))
        pt = torch.nn.functional.softmax(p_softmax)
        entropy = ce * (alpha * ((1 - pt) ** gamma))
        entropy = torch.sum(entropy, dim=1)
        return entropy
    ...
    def train(self, opt):
        # src, tar dataloaders
        if opt.sub_valid_data is not None:
            src_dataset, tar_dataset, valid_loader, sub_valid_loader = self.dataloader(opt)
        else:
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            # print(f"SOURCE IMAGE: {src_image.shape}")
            # print(f"SOURCE TEXT: {src_text} - {src_text.shape} - {src_length}")
            ...
            # print(f"SOURCE PREDCTION: {src_preds} - {src_preds.shape}")
            # print(f"GROUNDTRUTH: {target} - {target.shape}")
            
            logits = src_preds.view(-1, src_preds.shape[-1])
            gt = target.contiguous().view(-1)

            # print(f"LOGITS: {logits} - {logits.shape}")
            # print(f"GT: {gt} - {gt.shape}")

            src_cls_loss = self.criterion(logits, gt)
            # print(f"CLASSIFICATION: {src_cls_loss}")
            ...
            # print(f"TARGET PREDICTION: {tar_preds} - {tar_preds.shape}")

            # self-paced procedure
            # print(f"EM_LOSS: {tar_em_loss} - {tar_em_loss.shape}")
            # print(f"FOCAL_LOSS: {tar_focal_loss} - {tar_focal_loss.shape}")
            # print(f"TARGET FOCAL ENTROPY: {tar_em_loss} - {tar_em_loss.shape}")

            # self-paced procedure
            if opt.entropy_type == 'CE':
                tar_em_loss = self.get_batch_entropy_loss(tar_preds)
            elif opt.entropy_type == 'FE':
                tar_em_loss = self.entropy_focal(tar_preds, alpha=opt.focal_alpha, gamma=opt.focal_gamma)
            else: raise ValueError(f"{opt.entropy_type} not Implemented")
            ...
            if tar_portion < 1.0 :
                ...
                for c in class_set: 
                    ...
                    # print(f"TOP {k} LOWEST FOR ENTROPY: {choosed_ent}")
                    choosed_ent_pool = torch.cat((choosed_ent_pool, choosed_ent), 0)
                # print(f"ENTROPY POOL: {choosed_ent_pool} - {choosed_ent_pool.shape}")
            loss = (src_cls_loss.mean() * src_lambda) + (tar_em_loss.mean() * tar_lambda)
            ...
            if step % opt.valInterval == 0:
                ...
                with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a') as log:
                    train_log = f'[{step}/{opt.num_iter}] Loss: {loss_avg.val():0.5f} elapsed_time: {elapsed_time:0.5f} '
                    train_log += f"CLS_Loss: {cls_loss_avg.val():0.5f} EM_Loss: {em_loss_avg.val():0.5f}"
                    log.write(train_log + "\n")
                    loss_avg.reset()
                    cls_loss_avg.reset()
                    ...
                    with torch.no_grad():
                        ...
                        if opt.sub_valid_data is not None:
                            # Log sub validation dataset
                            if opt.sub_valid_data is not None:
                                sub_valid_loss, sub_accuracy, sub_norm_ED, sub_preds, sub_labels, _, _ = validation(
                                    self.model, self.criterion, sub_valid_loader, self.converter, opt
                                )
                    ...
                    if opt.sub_valid_data is not None:
                        log.write("-" * 80 + "\n")
                        self.print_prediction_result(sub_preds, sub_labels, log)
                        sub_valid_log = f'[{step}/{opt.num_iter}] SECOND valid loss: {sub_valid_loss:0.5f}'
                        sub_valid_log += f' SECOND accuracy: {sub_accuracy:0.3f}, SECOND norm_ED: {sub_norm_ED:0.2f}'
                        print(sub_valid_log)                    
                        log.write(sub_valid_log + "\n")
                    else:
                        valid_log = f'[{step}/{opt.num_iter}] MAIN valid loss: {valid_loss:0.5f}'
                        valid_log += f' MAIN accuracy: {current_accuracy:0.3f}, MAIN norm_ED: {current_norm_ED:0.2f}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_lambda', type=float, default=1.0,
                        help='the weight of supervised loss')
    parser.add_argument('--focal_alpha', type=float, default=0.2,
                        help='Alpha coefficient for focal entropy')
    parser.add_argument('--focal_gamma', type=int, default=1,
                        help='Gamma values for focal entropy, For CE LOSS: Alpha=1 - Gamma=0')
    parser.add_argument('--loss_focal_gamma', type=int, default=2,
                        help='Gamma values for focal loss')
    parser.add_argument('--entropy_type', required=True, type=str, choices=['CE', 'FE'], default='FE',
                        help='Entropy Minimization type - Choose: CE - FE')
    parser.add_argument('--loss_type', required=True, type=str, choices=['CELoss', 'FocalLoss'], default='CELoss',
                        help='Loss type for model. Choose: CELoss - FocalLoss')
    parser.add_argument('--sub_valid_data', type=str, default=None,
                        help='Observing loss knowledge with secondary valid dataset')
