...
class Batch_Balanced_Dataset(object):
    ...
    def __init__(self, opt, train_data, select_data, batch_ratio,is_shuffle=True):
        """
        ...
        for selected_d, batch_ratio_d in zip(select_data, batch_ratio):
            ...
            print('-' * 80)
            _dataset = hierarchical_dataset(root=train_data, opt=opt, select_data=[selected_d])
            ...
            self.total_data_size += len(_dataset)
            print(
...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        self.root = root
        self.opt = opt
        self.env = lmdb.open(root, max_readers=32, readonly=True, lock=False, readahead=False,
                             meminit=False)
        if not self.env:
        ...
        with self.env.begin(write=False) as txn:
            ...
            if self.opt.data_filtering_off:
                ...
                print(f"FILTERED: {self.filtered_index_list}")
                print(f"FINAL SAMPLES: {self.nSamples}")
